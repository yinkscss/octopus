...
- [x] All agent-generated alarms fire at the correct times
- [x] Alarm labels contain the user's own goal language — no generic text
- [x] Snooze fires exactly one follow-up — no infinite snooze loop
- [x] Alarms survive a Mac restart (persisted via LaunchAgent)
- [x] Acknowledgement data is written to SQLite with timestamp
